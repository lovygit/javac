...
import javac.tree.JCTree.JCTypeCast;
...
import javac.code.Type.UndetVar.InferenceBound;
import javac.code.Symbol.*;
import javac.comp.DeferredAttr.AttrMode;
import javac.comp.Infer.GraphSolver.InferenceGraph;
import javac.comp.Infer.GraphSolver.InferenceGraph.Node;
import javac.comp.Resolve.InapplicableMethodException;
import javac.comp.Resolve.VerboseResolutionMode;
import javac.util.GraphUtils.TarjanNode;
...
public class Infer {
    ...
    */
    ...
    interface GraphStrategy {
         ...
         */
        GraphSolver.InferenceGraph.Node pickNode(GraphSolver.InferenceGraph g) throws NodeNotFoundException;
        /**
    ...
    abstract class BestLeafSolver extends LeafSolver {
        ...
        Pair<List<GraphSolver.InferenceGraph.Node>, Integer> computeTreeToLeafs(GraphSolver.InferenceGraph.Node n) {
            Pair<List<GraphSolver.InferenceGraph.Node>, Integer> cachedPath = treeCache.get(n);
            if (cachedPath == null) {
                //cache miss
                if (n.isLeaf()) {
                    //if leaf, stop
                    cachedPath = new Pair<List<GraphSolver.InferenceGraph.Node>, Integer>(List.of(n), n.data.length());
                } else {
                    //if non-leaf, proceed recursively
                    Pair<List<GraphSolver.InferenceGraph.Node>, Integer> path = new Pair<List<GraphSolver.InferenceGraph.Node>, Integer>(List.of(n), n.data.length());
                    for (GraphSolver.InferenceGraph.Node n2 : n.getAllDependencies()) {
                        ...
                        path = new Pair<List<GraphSolver.InferenceGraph.Node>, Integer>(
        ...
        final Map<GraphSolver.InferenceGraph.Node, Pair<List<GraphSolver.InferenceGraph.Node>, Integer>> treeCache =
                new HashMap<GraphSolver.InferenceGraph.Node, Pair<List<GraphSolver.InferenceGraph.Node>, Integer>>();

        /** constant value used to mark non-existent paths */
        final Pair<List<GraphSolver.InferenceGraph.Node>, Integer> noPath =
                new Pair<List<GraphSolver.InferenceGraph.Node>, Integer>(null, Integer.MAX_VALUE);

        /**
         * Pick the leaf that minimize cost
         */
        @Override
        public GraphSolver.InferenceGraph.Node pickNode(final GraphSolver.InferenceGraph g) {
            treeCache.clear(); //graph changes at every step - cache must be cleared
            Pair<List<GraphSolver.InferenceGraph.Node>, Integer> bestPath = noPath;
