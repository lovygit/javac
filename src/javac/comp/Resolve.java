...
import source.tree.MemberReferenceTree.ReferenceMode;
import javac.api.Formattable.LocalizedString;
import javac.code.*;
import javac.code.Symbol.*;
import javac.code.Type.*;
import javac.comp.Attr.ResultInfo;
import javac.comp.Check.CheckContext;
import javac.comp.DeferredAttr.AttrMode;
import javac.comp.DeferredAttr.DeferredAttrContext;
import javac.comp.DeferredAttr.DeferredType;
import javac.comp.Infer.InferenceContext;
import javac.comp.Infer.FreeTypeListener;
import javac.comp.Resolve.MethodResolutionContext.Candidate;
import javac.comp.Resolve.MethodResolutionDiagHelper.DiagnosticRewriter;
import javac.comp.Resolve.MethodResolutionDiagHelper.Template;
import javac.jvm.*;
import javac.main.Option;
import javac.tree.*;
import javac.tree.JCTree.*;
import javac.tree.JCTree.JCMemberReference.ReferenceKind;
import javac.tree.JCTree.JCPolyExpression.*;
import javac.util.*;
import javac.util.JCDiagnostic.DiagnosticFlag;
import javac.util.JCDiagnostic.DiagnosticPosition;
import javac.util.JCDiagnostic.DiagnosticType;
...
import static javac.code.Flags.*;
import static javac.code.Flags.BLOCK;
import static javac.code.Kinds.*;
import static javac.code.Kinds.ERRONEOUS;
import static javac.code.TypeTag.*;
import static javac.comp.Resolve.MethodResolutionPhase.*;
import static javac.tree.JCTree.Tag.*;
import static javac.comp.Resolve.MethodResolutionPhase.BASIC;
import static javac.comp.Resolve.MethodResolutionPhase.BOX;
import static javac.comp.Resolve.MethodResolutionPhase.VARARITY;
...
public class Resolve {
    ...
    void reportVerboseResolutionDiagnostic(DiagnosticPosition dpos, Name name, Type site,
        ...
        for (MethodResolutionContext.Candidate c : currentResolutionContext.candidates) {
        ...
        private boolean canIgnore(Symbol s) {
            switch (s.kind) {
                ...
                case WRONG_MTH:
                    ...
                    return new MethodResolutionDiagHelper.Template(MethodCheckDiag.ARITY_MISMATCH.regex())
 ...
 ****************************************************************************/
    ...
    class InapplicableSymbolError extends ResolveError {
        ...
        JCDiagnostic getDiagnostic(JCDiagnostic.DiagnosticType dkind,
            ...
            if (syms.operatorNames.contains(name)) {
                ...
                if (compactMethodDiags) {
                    for (Map.Entry<MethodResolutionDiagHelper.Template, MethodResolutionDiagHelper.DiagnosticRewriter> _entry :
        ...
        protected Pair<Symbol, JCDiagnostic> errCandidate() {
            ...
            for (MethodResolutionContext.Candidate c : resolveContext.candidates) {
        ...
        //where
            ...
            Map<Symbol, JCDiagnostic> filterCandidates(Map<Symbol, JCDiagnostic> candidatesMap) {
                ...
                for (Map.Entry<Symbol, JCDiagnostic> _entry : candidatesMap.entrySet()) {
                    ...
                    if (!new MethodResolutionDiagHelper.Template(MethodCheckDiag.ARITY_MISMATCH.regex()).matches(d)) {
