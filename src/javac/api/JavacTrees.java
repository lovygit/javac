...
import source.doctree.DocCommentTree;
import source.doctree.DocTree;
import source.tree.CatchTree;
import source.tree.CompilationUnitTree;
import source.tree.Scope;
import source.tree.Tree;
import source.util.DocSourcePositions;
import source.util.DocTreePath;
import source.util.DocTreeScanner;
import source.util.DocTrees;
import source.util.JavacTask;
import source.util.TreePath;
import javac.code.Flags;
import javac.code.Kinds;
import javac.code.Symbol;
import javac.code.Symbol.ClassSymbol;
import javac.code.Symbol.MethodSymbol;
import javac.code.Symbol.PackageSymbol;
import javac.code.Symbol.TypeSymbol;
import javac.code.Symbol.VarSymbol;
import javac.code.Type;
import javac.code.Type.ArrayType;
import javac.code.Type.ClassType;
import javac.code.Type.ErrorType;
import javac.code.Type.UnionClassType;
import javac.code.Types;
import javac.code.Types.TypeRelation;
import javac.comp.Attr;
import javac.comp.AttrContext;
import javac.comp.Enter;
import javac.comp.Env;
import javac.comp.MemberEnter;
import javac.comp.Resolve;
import javac.model.JavacElements;
import javac.processing.JavacProcessingEnvironment;
import javac.tree.DCTree;
import javac.tree.DCTree.DCBlockTag;
import javac.tree.DCTree.DCDocComment;
import javac.tree.DCTree.DCEndPosTree;
import javac.tree.DCTree.DCErroneous;
import javac.tree.DCTree.DCIdentifier;
import javac.tree.DCTree.DCParam;
import javac.tree.DCTree.DCReference;
import javac.tree.DCTree.DCText;
import javac.tree.EndPosTable;
import javac.tree.JCTree;
import javac.tree.JCTree.*;
import javac.tree.TreeCopier;
import javac.tree.TreeInfo;
import javac.tree.TreeMaker;
import javac.util.Abort;
import javac.util.Assert;
import javac.util.Context;
import javac.util.JCDiagnostic;
import javac.util.List;
import javac.util.ListBuffer;
import javac.util.Log;
import javac.util.Name;
import javac.util.Names;
import javac.util.Pair;
import javac.util.Position;
import static javac.code.TypeTag.*;
...
public class JavacTrees extends DocTrees {
    ...
    private VarSymbol searchField(ClassSymbol tsym, Name fieldName, Set<ClassSymbol> searched) {
        ...
        for (javac.code.Scope.Entry e = tsym.members().lookup(fieldName);
    ...
    MethodSymbol findConstructor(ClassSymbol tsym, List<Type> paramTypes) {
        for (javac.code.Scope.Entry e = tsym.members().lookup(names.init);
                e.scope != null; e = e.next()) {
    ...
    private MethodSymbol searchMethod(ClassSymbol tsym, Name methodName,
        ...
        javac.code.Scope.Entry e = tsym.members().lookup(methodName);
    ...
    public boolean isAccessible(Scope scope, Element member, DeclaredType type) {
        if (scope instanceof JavacScope
                ...
                && type instanceof javac.code.Type) {
            Env<AttrContext> env = ((JavacScope) scope).env;
            return resolve.isAccessible(env, (javac.code.Type)type, (Symbol)member, true);
        } else
            return false;
    }
...
//                    System.err.println("DEFAULT: " + tree.getKind());
    ...
    public TypeMirror getOriginalType(javax.lang.model.type.ErrorType errorType) {
        if (errorType instanceof javac.code.Type.ErrorType) {
            return ((javac.code.Type.ErrorType)errorType).getOriginalType();
        }

        return javac.code.Type.noType;
    }
    ...
    public void printMessage(Diagnostic.Kind kind, CharSequence msg,
            source.tree.Tree t,
            source.tree.CompilationUnitTree root) {
        printMessage(kind, msg, ((JCTree) t).pos(), root);
    }

    public void printMessage(Diagnostic.Kind kind, CharSequence msg,
            source.doctree.DocTree t,
            source.doctree.DocCommentTree c,
            source.tree.CompilationUnitTree root) {
